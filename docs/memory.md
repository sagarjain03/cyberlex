...
| Date | Item | Notes |
|------|------|-------|
| 2026-08-16 | **Removed `lenis`; scrolling is native again** | User reported the mouse wheel not working. Two compounding defects: Lenis's required stylesheet was **never imported**, and `<html>` carried `h-full` (`height: 100%`) while documents run to ~5500px — the exact condition Lenis's own CSS exists to fix (`html.lenis, html.lenis body { height: auto }`). Rather than patch it, removed the library: smooth scroll is decoration, and the Obsidian rewrite already deleted every other decorative effect. Verified: 8 routes × 2 viewports all scroll. New binding rule — **never hijack browser-native input behaviour for aesthetics** (`docs/rules.md` §1.4 rule 7). |
...
| 2026-08-15 | **Phase 1 — types & data layer** | `types/` (9 files), `data/` (6 files + README), `lib/data/` (7 files), `lib/scoring/` (3 files), `lib/format.ts`. 10 jurisdictions · 27 laws · 12 tracker entries · 6 AI-crime techniques · 60 coverage cells · 35 primary sources. Dashboard now renders entirely from the repository. |
